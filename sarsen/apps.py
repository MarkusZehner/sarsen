...
def do_terrain_correction(
        ...
        simulated_precalculated: Optional[str] = None,
        correct_radiometry: Optional[str] = None,
    ...
    if correct_radiometry is not None and simulated_precalculated is None:
    ...
    if simulated_precalculated is not None:
        simulated_beta_nought = xr.open_dataarray(simulated_precalculated).squeeze()
    ...
    if correct_radiometry is not None or simulated_precalculated is not None:
...
def terrain_correction(
    ...
    :param grouping_area_factor: is a tuple of floats greater than 1. The default is `(1.0, 1.0)`.
    ...
    :param oversampling: is a tuple of floats greater than 1. The default is `(1, 1)`.
    The `oversampling` can be increased to apply oversampling on the DEM and possibly improve the quality of the final result.
    ...
    geocoded, simulated_beta_nought = do_terrain_correction(
        ...
        simulated_precalculated=simulated_precalculated,
    ...
    if output_urlpath is None:
        assert simulated_beta_nought is not None
        print('simulated_beta_nought')
        return simulated_beta_nought
    
    logger.info("save output")
    
    if log_output:
        geocoded.values = 10 * np.log10(geocoded.values)
